...
//------------------------------------------------------------------------------
	...
	*/
	...
	this.buildTags = function() {
		...
		$.each(this.tags.items, function(i, tag){
			if(tag.controlled==0){
				...
				self.buildRequiredTag(tag);
			}
		});
	}
	//Building function for NOT-required Tags
	this.buildFreeTag = function(tag) {
		...
		//Checking if we haven't add tag from tagCloud via suggester => in that case we have to assing selectedTag class to the selected tag in a tagCloud
		if(this.optionalTags[tag.uri]){
			this.optionalTags[tag.uri].addClass("selectedTag");
		}
	...
	//Building function for REQUIRED Tags
	this.buildRequiredTag = function(tag) {
		// Ignore, if we already have this tag in the UI
		...
		//Checking if TAG's category-list exists (category's 'required' is 'true')
		if($(this.CS).find(".tagsEditor_requiredTags").children("."+tag.prefix+"_cont").index()==0){
			//Category-list exists so just put TAG inside
			$(this.CS).find(".tagsEditor_requiredTags").children("."+tag.prefix+"_cont").append(tagEl);
		}
		//Category-list doesn't exists (category's 'required' is 'false') so we have to make additional list for it
		else{
			//We know the 'prefix' for the category but we don't know the 'label' whitch we need
			for(var j = 0;j<this.requiredCategoryOptions.length;j++){
				...
				if(this.requiredCategoryOptions[j].prefix == tag.prefix){
					$(this.CS).find(".tagsEditor_requiredTags").append("<div class=\""+this.requiredCategoryOptions[j].prefix+"_cont category_item\">"+this.requiredCategoryOptions[j].label+": </div>");
					$(this.CS).find(".tagsEditor_requiredTags").children("."+tag.prefix+"_cont").append(tagEl);
				}
			}
		}
		...
		// Keep reference to the DOM element
		this.requiredTags[tag.uri] = tagEl;
	}
	...
	this.removeTag = function(tagUri) {
		//Checking if tag is 'recommended' = is represented in both freeTags and optionalTags
		if(this.freeTags[tagUri]&&this.optionalTags[tagUri]){
			this.optionalTags[tagUri].removeClass("selectedTag");
		...
		else if(this.requiredTags[tagUri]){
			// Remove the required tag from the UI aggressively;
			this.requiredTags[tagUri].remove();
			// Also remove the reference to removed DOM element
			delete this.requiredTags[tagUri];
		}
		...
		*/
			...
			for(var i = 0;i<this.requiredCategoryOptions.length;i++){
				if(this.requiredCategoryOptions[i].required == true){
					requiredTagsContainer.append("<div class=\""+this.requiredCategoryOptions[i].prefix+"_cont category_item\">"+this.requiredCategoryOptions[i].label+": </div>");
				}
			}
			//END OF NEW CODE
	...
	this.buildRecommendedTags = function(data){
		...
		$.each(data.optional, function(i, tag){
			...
			if (fontSize >= self.MIN_TAG_FONT_SIZE) {
				...
				self.optionalTags[tag.uri] = tagEl;
...
//------------------------------------------------------------------------------
	...
	this.deploy = function(){
		...
		this.suggester.onAddTag = jQuery.proxy(function(label,uri){
			var tag = {
				label: label,
				uri: uri
			}
