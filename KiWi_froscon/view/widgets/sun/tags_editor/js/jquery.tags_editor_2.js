...
//        this.RESOURCE_URI = "http://kiwi.sunsolutioncenter.de/KiWi/content/65834035-935c-4d78-9fe2-e544190d1bff";
	...
	// NEW - Map of "tag" labels to DOM elements representing required tags
	this.controlledTags = {};
...
//------------------------------------------------------------------------------
	...
	this.onSuccessFetchTags = function(data){
		this.hideProgress();
		this.suggester.setSelectedTags(data.items);
		this.tags = data
		...
		if (this.tags && this.requiredCategoryOptions) {
			...
			/*
				for(var i = 0;i<this.tags.length;i++){
					//Checking if TAG is controlled
					if(this.tags[i].controlled == 1){
						//Checking if TAG's category-list exists (category's 'required' is 'true')
						if($(this.CS).find(".tagsEditor_requiredTags").children("."+this.tags[i].prefix+"_cont").index()==0){
							//Category-list exists so just put TAG inside
							$(this.CS).find(".tagsEditor_requiredTags").children("."+this.tags[i].prefix+"_cont").append("<span class=\"requiredTagsItem\">"+this.tags[i].label+"<img src='./img/x.gif' /></span>");
						}
						//Category-list doesn't exists (category's 'required' is 'false') so we have to make additional list for it
						else{
							//We know the 'prefix' for the category but we don't know the 'label' whitch we need
							for(var j = 0;j<this.requiredCategoryOptions.length;j++){
								//We got the label - now we can make wanted additional category-list and pust TAG inside
								if(this.requiredCategoryOptions[j].prefix == this.tags[i].prefix){
									$(this.CS).find(".tagsEditor_requiredTags").append("<div class=\""+this.requiredCategoryOptions[j].prefix+"_cont category_item\">"+this.requiredCategoryOptions[j].label+": <span class=\"requiredTagsItem\">"+this.tags[i].label+"<img src='./img/x.gif' /></span></div>");
								}
							}
						}
						...
						tagEl.html(this.tags[i].label + "<img src='./img/x.gif' /> ");
						freeTagsContainer.append(tagEl);
					}
				}
			
			$(this.CS).find(".tagsEditor_requiredTags").children(".category_item").children("span").children("img").click(function(){
				//ADD REMOVE ICON HANDLER from controlled required tags
				alert("required tags");
			})
			
			$(this.CS).find(".tagsEditor_freeTags").children("span").children("img").click(function(){
				alert("free tags");
				//ADD REMOVE ICON HANDLER from not-controlled required tags (freetags)
			})
			*/
			////END OF NEW CODE
		}
...
//------------------------------------------------------------------------------
	...
	this.addTag = function(tagsString){
		...
		$.each(tags, function(i, tag){
			...
			if (self.recommendedTags[tag]) {
				self.recommendedTags[tag].addClass("selectedTag");
			}
		});
	...
	*/
	...
	this.buildFreeTag = function(tag) {
		...
		if(this.recommendedTags[tag.uri]){
			this.recommendedTags[tag.uri].addClass("selectedTag");
		}
	...
	this.buildCategoryTag = function(tag) {
		...
		//Checking if TAG's category-list exists (category's 'required' is 'true')
		if($(this.CS).find(".tagsEditor_requiredTags").children("."+tag.prefix+"CategoryContainer").length){
			//Category-list exists so just put TAG inside
			$(this.CS).find(".tagsEditor_requiredTags").children("."+tag.prefix+"CategoryContainer").children(".categoryTags").append(tagEl);
			self.checkEmptyCategories($(this.CS).find(".tagsEditor_requiredTags").children("."+tag.prefix+"CategoryContainer").children(".categoryTags"));
		}
		//Category-list doesn't exists (category's 'required' is 'false') so we have to make additional room for it
			...
			for(var j = 0;j<this.requiredCategoryOptions.length;j++){
				//We got the label - now we can make wanted additional category-list and pust TAG inside
				if(this.requiredCategoryOptions[j].prefix == tag.prefix){
					//$(this.CS).find(".tagsEditor_requiredTags").append("<div class=\""+tag.prefix+"CategoryContainer categoryContainer\"> <div class=\"categoryName\">"+this.requiredCategoryOptions[j].label+":</div> <div class=\"categoryTags\"></div> </div>");

					//We use for this newly made function
					this.buildCategory(tag.prefix,this.requiredCategoryOptions[j].label);
					$(this.CS).find(".tagsEditor_requiredTags").children("."+tag.prefix+"CategoryContainer").children(".categoryTags").append(tagEl);
	...
	this.removeTag = function(tagUri) {
		//Checking if tag is 'recommended' = is represented in both freeTags and recommendedTags
		if(this.freeTags[tagUri]&&this.recommendedTags[tagUri]){
			this.recommendedTags[tagUri].removeClass("selectedTag");
...
//------------------------------------------------------------------------------
		...
		*/
			...
			for(var i = 0;i<this.requiredCategoryOptions.length;i++){
				if(this.requiredCategoryOptions[i].required == true){
					self.buildCategory(this.requiredCategoryOptions[i].prefix,this.requiredCategoryOptions[i].label);
				}
			}
	...
	this.buildCategory = function(prefix,label){
		...
		requiredTagsContainer.append(
			"<div class=\""+prefix+"CategoryContainer categoryContainer\"> <div class=\"categoryName\">"+label+":</div> <div class=\"categoryTags empty\">  <span class=\"add\"><a href=\"\">add</a> <button type=\"button\">+</button></span><span class=\"none\">none</span>  </div> <div class=\"categoryClear\"></div> </div>");
	}
	...
	this.setCategoryTag = function(uri) {
		var tag = this.controlledTags[uri];
		if (!tag)
			return;
...
//------------------------------------------------------------------------------
	...
	this.buildRecommendedTags = function(data){
		...
		$.each(data.optional, function(i, tag){
			...
			if (fontSize >= self.MIN_TAG_FONT_SIZE) {
				...
				self.recommendedTags[tag.uri] = tagEl;
	...
	this.bindTagsHandlers = function(){
		...
		*/
			...
			*/
			...
			if (el.attr("tagName") == "A") {
				...
				if (el.hasClass("selectedTag")) {
					...
					self.suggester.selectSuggestion(el.attr("tagUri"));
				} else {
					//Tag hasn't been selected yet => build up new free tag
		...
		//3rd requiredTagsHandlers
		var requiredTagsContainer = $(this.CS).find(".tagsEditor_requiredTags");
		requiredTagsContainer.click(function(event){
			var el = $(event.target);
			if (el.attr("tagName") == "IMG") {
				var tagUri = $(el.parent()[0]).attr("taguri");
				var categoryContainer = el.parent().parent();
				self.removeTag(tagUri);
				self.suggester.onTagRemoved(tagUri);
				self.checkEmptyCategories(categoryContainer);
				event.stopPropagation();
			}
		});
...
//------------------------------------------------------------------------------
	...
	this.deploy = function(){
		...
		this.suggester = new jQuery.Suggester();
		this.suggester.addTag = this.addTag;

		this.suggester.onRemoveTag = jQuery.proxy(function(uri){
			this.removeTag(uri);
		},this);

		this.suggester.onAddTag = jQuery.proxy(function(tag){
			if(tag.controlled==1){
