...
final class JDBCParser {
    ...
    /**
     * The value for the JNDI node corresponding to to the KiWi
     * application.
     */
    private static final String CEQ_JNDI_NAME = "CEQDatasource"; //$NON-NLS-1$

    /**
     * The value for the JNDI node corresponding to to the CEQ
     * application.
     */
    private static final String KIWI_JNDI_NAME = "KiWiDatasource"; //$NON-NLS-1$

    /**
     * The DOM for the JDBC configuration file (data source
     * file).
     */
    private final Document document;

    /**
     * Contains all the JDBC setting for CEQ.
     */
    private Node ceqNode;
     ...
     * 
     * @param document the DOM correspond to the data source xml
    ...
    JDBCParser(Document document) {
        ...
        for (int index = 0; index < nodes.getLength(); index++) {
            ...
            if (CEQ_JNDI_NAME.equals(textContent)) {
                ceqNode = item.getParentNode();
                continue;
            }
        }
    }
    ...
    String getKiWiJndiName() {
        final NodeList elements =
                ((Element) kiwiNode).getElementsByTagName(JNDI_NODE_NAME);
    ...
    /**
     * Extract the text content for a given child node, the node
     * is located after its tag name and it is searched from a
     * given parent node.
     * 
     * @param parentNode the parent node - from here is the
     *            search done.
     * @param nodeName the node name to search.
     * @return the text content for the
     */
    private String getNodeContent(Node parentNode, String nodeName) {
        final NodeList elements =
                ((Element) parentNode).getElementsByTagName(nodeName);
    ...
    String getKiWiConnectionURL() {
        final String result =
                getNodeContent(kiwiNode, CONNECTION_URL_NODE_NAME);
        return result;
    }
    ...
    /**
     * Returns the JDBC driver name used by the CEQ database.
     * 
     * @return the JDBC driver name used by the CEQ application.
     */
    String getCEQDriver() {
        final String result = getNodeContent(ceqNode, DRIVER_CLASS_NODE_NAME);
        return result;
    }

    /**
     * Returns the URL used to connect to CEQ database.
     * 
     * @return the URL used by the CEQ database.
     */
    String getCEQConnectionURL() {
        final String result = getNodeContent(ceqNode, CONNECTION_URL_NODE_NAME);
        return result;
    }

    void setCEQConnectionURL(String url) {
        setNodeContent((Element) ceqNode, CONNECTION_URL_NODE_NAME, url);
    }

    /**
     * Returns the URL used by the CEQ database..
     * 
     * @return the JDBC driver name used by the KiWi application.
     */
    String getCEQUser() {
        final String result = getNodeContent(ceqNode, USER_NODE_NAME);
        return result;
    }

    void setCEQUser(String user) {
        setNodeContent((Element) ceqNode, USER_NODE_NAME, user);
    }

    /**
     * Returns the actual user's password for the CEQ database.
     * 
     * @return the actual user's password for the CEQ database.
     */
    String getCEQPassword() {
        final String result = getNodeContent(ceqNode, PASSWORD_NODE_NAME);
        return result;
    }

    void setCEQPassword(String passwd) {
        setNodeContent((Element) ceqNode, PASSWORD_NODE_NAME, passwd);
    }
